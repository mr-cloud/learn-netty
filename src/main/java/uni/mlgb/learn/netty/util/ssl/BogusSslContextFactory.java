package uni.mlgb.learn.netty.util.ssl;
...
public class BogusSslContextFactory {
     ...
     */
    private static final String BOGUS_KEYSTORE = "bogus.p12";
    ...
    // convert to pkcs12
    /*
    keytool -importkeystore -srckeystore bogus.cert -destkeystore bogus.cert -deststoretype pkcs12
     */
    ...
    private static SSLContext createBougusServerSslContext() throws GeneralSecurityException, IOException {
        ...
        InputStream in = null;
        try {
            in = Thread.currentThread().getContextClassLoader().getResourceAsStream(BOGUS_KEYSTORE);
            ks.load(in, BOGUS_PW);
        } finally {
            if (in != null) {
                try {
